...
void Lifecycle::TransitionToServing() noexcept {
    ...
    state_.compare_exchange_strong(
        expected, State::kServing, std::memory_order_acq_rel, std::memory_order_acquire);
    ...
    if (health_) {
        ...
        if (current == Health::Status::kUnspecified || current == Health::Status::kServing) {
...
bool Lifecycle::SignalDrain() noexcept {
    ...
    while (true) {
        ...
        if (state_.compare_exchange_weak(
                expected, State::kDraining, std::memory_order_acq_rel, std::memory_order_acquire)) {
            break;
        }
