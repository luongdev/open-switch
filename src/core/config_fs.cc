...
bool LoadConfigFromFile(const char* xml_file_name, Config& out) {
    ...
    StringParam sp_cert{};   sp_cert.target = &out.grpc_tls_cert_path;
    StringParam sp_key{};    sp_key.target  = &out.grpc_tls_key_path;
    StringParam sp_ca{};     sp_ca.target   = &out.grpc_tls_ca_path;
    StringParam sp_log{};    sp_log.target  = &out.log_level;
    StringParam sp_acl{};    sp_acl.target  = &out.tenant_allowed_contexts;
    std::string pii_pipe;
    StringParam sp_pii{};    sp_pii.target  = &pii_pipe;
    ...
    switch_xml_config_int_options_t opt_ge1{};        opt_ge1.enforce_min = SWITCH_TRUE; opt_ge1.min = 1;
    switch_xml_config_int_options_t opt_ge16{};       opt_ge16.enforce_min = SWITCH_TRUE; opt_ge16.min = 16;
    switch_xml_config_int_options_t opt_ge64{};       opt_ge64.enforce_min = SWITCH_TRUE; opt_ge64.min = 64;
    switch_xml_config_int_options_t opt_ge256{};      opt_ge256.enforce_min = SWITCH_TRUE; opt_ge256.min = 256;
    ...
    switch_xml_config_item_t instructions[] = {
        // gRPC server
        SWITCH_CONFIG_ITEM_CALLBACK(
            "grpc_listen_address", SWITCH_CONFIG_CUSTOM, CONFIG_RELOADABLE,
            nullptr, "0.0.0.0:50061",
            &StringCallback, &sp_listen, "host:port", "gRPC server listen address"),
        SWITCH_CONFIG_ITEM(
            "grpc_max_concurrent_streams", SWITCH_CONFIG_INT, CONFIG_RELOADABLE,
            &int_max_streams, reinterpret_cast<const void*>(static_cast<std::intptr_t>(256)),
            &opt_ge1, "uint", "Max concurrent gRPC streams"),
        SWITCH_CONFIG_ITEM(
            "grpc_drain_deadline_seconds", SWITCH_CONFIG_INT, CONFIG_RELOADABLE,
            &int_drain_deadline, reinterpret_cast<const void*>(static_cast<std::intptr_t>(2)),
            &opt_ge1, "seconds", "gRPC server shutdown deadline"),

        // TLS
        SWITCH_CONFIG_ITEM_CALLBACK(
            "grpc_tls_cert_path", SWITCH_CONFIG_CUSTOM, CONFIG_RELOADABLE,
            nullptr, "",
            &StringCallback, &sp_cert, "path", "PEM server cert (empty = TLS off)"),
        SWITCH_CONFIG_ITEM_CALLBACK(
            "grpc_tls_key_path", SWITCH_CONFIG_CUSTOM, CONFIG_RELOADABLE,
            nullptr, "",
            &StringCallback, &sp_key, "path", "PEM server private key"),
        SWITCH_CONFIG_ITEM_CALLBACK(
            "grpc_tls_ca_path", SWITCH_CONFIG_CUSTOM, CONFIG_RELOADABLE,
            nullptr, "",
            &StringCallback, &sp_ca, "path", "PEM CA bundle (empty = no mTLS)"),

        // Event plane
        SWITCH_CONFIG_ITEM(
            "event_ring_capacity_tier1", SWITCH_CONFIG_INT, CONFIG_RELOADABLE,
            &int_ring_t1, reinterpret_cast<const void*>(static_cast<std::intptr_t>(16384)),
            &opt_ge256, "events", "Tier-1 ring capacity"),
        SWITCH_CONFIG_ITEM(
            "event_ring_capacity_tier2", SWITCH_CONFIG_INT, CONFIG_RELOADABLE,
            &int_ring_t2, reinterpret_cast<const void*>(static_cast<std::intptr_t>(8192)),
            &opt_ge256, "events", "Tier-2 ring capacity"),
        SWITCH_CONFIG_ITEM(
            "event_ring_capacity_tier3", SWITCH_CONFIG_INT, CONFIG_RELOADABLE,
            &int_ring_t3, reinterpret_cast<const void*>(static_cast<std::intptr_t>(4096)),
            &opt_ge256, "events", "Tier-3 ring capacity"),
        SWITCH_CONFIG_ITEM(
            "subscriber_send_queue_capacity", SWITCH_CONFIG_INT, CONFIG_RELOADABLE,
            &int_sub_send, reinterpret_cast<const void*>(static_cast<std::intptr_t>(4096)),
            &opt_ge64, "events", "Per-subscriber send queue cap"),
        SWITCH_CONFIG_ITEM(
            "max_subscribers", SWITCH_CONFIG_INT, CONFIG_RELOADABLE,
            &int_max_sub, reinterpret_cast<const void*>(static_cast<std::intptr_t>(16)),
            &opt_ge1, "streams", "Max concurrent SubscribeEvents streams"),

        // Idempotency
        SWITCH_CONFIG_ITEM(
            "idempotency_ttl_seconds", SWITCH_CONFIG_INT, CONFIG_RELOADABLE,
            &int_idemp_ttl, reinterpret_cast<const void*>(static_cast<std::intptr_t>(300)),
            &opt_ge1, "seconds", "Idempotency cache TTL"),
        SWITCH_CONFIG_ITEM(
            "idempotency_cache_capacity", SWITCH_CONFIG_INT, CONFIG_RELOADABLE,
            &int_idemp_cap, reinterpret_cast<const void*>(static_cast<std::intptr_t>(1500)),
            &opt_ge16, "entries", "Idempotency cache capacity"),
        SWITCH_CONFIG_ITEM(
            "idempotency_in_flight_max_wait_seconds", SWITCH_CONFIG_INT, CONFIG_RELOADABLE,
            &int_idemp_max_wait, reinterpret_cast<const void*>(static_cast<std::intptr_t>(30)),
            &opt_ge1, "seconds", "Idempotency in-flight shadow timeout"),

        // Drain
        SWITCH_CONFIG_ITEM(
            "drain_timeout_seconds", SWITCH_CONFIG_INT, CONFIG_RELOADABLE,
            &int_drain_to, reinterpret_cast<const void*>(static_cast<std::intptr_t>(30)),
            &opt_ge1, "seconds", "Drain max wait"),
        SWITCH_CONFIG_ITEM(
            "event_drain_timeout_seconds", SWITCH_CONFIG_INT, CONFIG_RELOADABLE,
            &int_evdrain_to, reinterpret_cast<const void*>(static_cast<std::intptr_t>(5)),
            &opt_ge1, "seconds", "Event-ring drain max wait"),

        // Terminate handler (default OFF per architecture.md §"Terminate handler chaining")
        SWITCH_CONFIG_ITEM(
            "osw_panic_on_unhandled", SWITCH_CONFIG_BOOL, CONFIG_RELOADABLE,
            &bool_panic, reinterpret_cast<const void*>(static_cast<std::intptr_t>(SWITCH_FALSE)),
            nullptr, "true|false", "Install std::set_terminate handler"),
        SWITCH_CONFIG_ITEM(
            "osw_install_signal_handlers", SWITCH_CONFIG_BOOL, CONFIG_RELOADABLE,
            &bool_sigh, reinterpret_cast<const void*>(static_cast<std::intptr_t>(SWITCH_FALSE)),
            nullptr, "true|false", "Install SIGSEGV/SIGABRT/SIGBUS handlers"),

        // Observability
        SWITCH_CONFIG_ITEM_CALLBACK(
            "log_level", SWITCH_CONFIG_CUSTOM, CONFIG_RELOADABLE,
            nullptr, "info",
            &StringCallback, &sp_log, "trace|debug|info|warn|error|crit", "Log level"),
        SWITCH_CONFIG_ITEM_CALLBACK(
            "pii_redaction_patterns", SWITCH_CONFIG_CUSTOM, CONFIG_RELOADABLE,
            nullptr, "",
            &StringCallback, &sp_pii, "pipe-separated regexes", "PII redaction patterns"),

        // Tenant ACL (W3 will consume tenant_allowed_contexts)
        SWITCH_CONFIG_ITEM_CALLBACK(
            "tenant_allowed_contexts", SWITCH_CONFIG_CUSTOM, CONFIG_RELOADABLE,
            nullptr, "",
            &StringCallback, &sp_acl, "tenant:ctx1,ctx2;...", "Per-tenant allowed dialplan contexts"),
    ...
    const switch_status_t st = switch_xml_config_parse_module_settings(
        xml_file_name, SWITCH_FALSE, instructions);
